...
import { AccessKey, InvalidAccessKeyError } from '../../../../src/domain/value-objects/access-key.js'
...
describe('AccessKey', () => {
  describe('create', () => {
    ...
    it('strips non-digit characters before validating', () => {
      const formatted = '3524 0312 3456 7800 0195 5500 1000 0001 2312 3456 7890'
    ...
    it('rejects a key with fewer than 44 digits', () => {
      const result = AccessKey.create('3524031234567800019555001000000123123456789')
      expect(result.ok).toBe(false)
      if (!result.ok) expect(result.error).toBeInstanceOf(InvalidAccessKeyError)
    })

    it('rejects a key with more than 44 digits', () => {
      const result = AccessKey.create('352403123456780001955500100000012312345678901')
      expect(result.ok).toBe(false)
    })
